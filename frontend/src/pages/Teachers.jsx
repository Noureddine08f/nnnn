...
const Teachers = () => {
  ...
  const [formData, setFormData] = useState({ name: '', max_hours: 18, color: '#3b82f6' }); const [editingId, setEditingId] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', max_hours: 18, color: '#3b82f6' }); setEditingId(null);
      fetchTeachers();
    } catch (error) {
  ...
  return (
    <div className="p-4">
      ...
      <form onSubmit={handleSubmit} className="mb-8 bg-white p-4 rounded shadow flex flex-wrap gap-4 items-end">
        ...
        {editingId && (
          <button type="button" onClick={() => { setEditingId(null); setFormData({ name: '', max_hours: 18, color: '#3b82f6' }); }} className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">            Cancel
          </button>
        )}
      </form>
      ...
      <div className="bg-white rounded shadow overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-400">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Max Hours</th>
